...
class DebateLLM: # openai chat abstraction
    def __init__(self, *, client: Optional[OpenAIClient] = None, model_name: Optional[str] = None, api_key: Optional[str] = None) -> None: # load prompts and config client
        ...
        self.client = client or self._initClient(api_key)

    def _initClient(self, api_key: Optional[str] = None) -> Optional[OpenAIClient]: # init openai client
        if OpenAI is None:
    ...
    def generate_tweet_reply(self, history: List[LLMMessage], topic: str) -> str:
        ...
        if not history:
             messages.append({"role": "user", "content": f"Debate this topic: {topic}"})

        if not self.client:
            return "I'm ready to debate! (Mock response)"
